...
export class SubscriptionsService {
  ...
  async CreateSubscription(subscriberPublicKey: string, amount: number, periodSeconds: number): Promise<any> {
	...
	async GetSubscription(subscriberPublicKey: string): Promise<any> {
		return await this.api.Call<any>('GET', `subscriptions/${subscriberPublicKey}`);
	}
	...
	async ChargeSubscription(subscriberPublicKey: string, feePayerPublicKey: string): Promise<any> {
	...
	async GetSubscriptionDebugInfo(subscriberPublicKey: string): Promise<any> {
		return await this.api.Call<any>('GET', `subscriptions/${subscriberPublicKey}/debug`);
	}
}
